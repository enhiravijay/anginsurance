...
public class QuotePage extends BasePage {
	...
	public static void main(String[] args) throws Exception {
		...
		driver.get("https://www.policyboss.com/car-insurance/quotes?SID=SRN-XXGJUSWJ-MBZA-QSFT-EMVA-PNNIXS8YCXAJ_5238383&ClientID=2");
		...
		System.out.println("value from main " + isInsPresent("Bajaj"));
		buyIns("tata");
		Thread.sleep(5000);
	...
	public static void buyIns(String svalue) {
		...
		for(int i = 0;i<lisofinsname.size();i++) {
			...
			String attval = tolele.get(i).getAttribute("id");
			//premlist.addAll(premlist2);
			if(inskeyvalue1.contains(somevalue)) {
				//div[contains(@id, 'insurer')]/div[4]/span[1]/div[1]
				WebElement elepre = driver.findElement(By.xpath("//div[@id='"+ attval +"' and @class = \"quoteBox ng-star-inserted\"]/div[@class='prem_det'][1]//span[contains(text(),\"₹\")][1]"));
				
				if(elepre.isDisplayed()) {
					elepre.click();
				}else {
					System.out.println("buy button Element is not displayed");
				}
